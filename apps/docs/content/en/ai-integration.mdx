...
routar is indexed on [Context7](https://context7.com/minr2kb/routar), so AI coding assistants with the Context7 MCP server configured (Cursor, Claude Code, Windsurf, …) can pull up-to-date routar docs directly into context — no manual copy-pasting of `llms.txt`.

Once the Context7 MCP server is set up, add "use context7" to your prompt and the assistant resolves routar's docs automatically. Without it, point the assistant at `/llms-full.txt` directly (see above).
