...
Each default value may be a **static object** or a **function** `(params, q) => options`, evaluated lazily per call. The function form receives:

- `q` — the **fully-built** `queries` object that `createQueries` returns, so every key helper (`.queryKey()`, `.mutationKey`, `.$key`) is available without circular-variable issues. Use it for `invalidates`.
- `params` — the call params for a query accessor, or `undefined` for a mutation accessor (mutations have no fixed call params).
...
export const todoQuery = createQueries(todoApi, {
  defaults: {
    ...
    // (params, q) => options — depends on the call params
    getList: (params, _q) => ({
      staleTime: params?.query?.done ? 60_000 : 0,
    }),

    // (_, q) => options — mutation: reference sibling key helpers off the built q
...
Unlike the previous top-level factory form (`createQueries(api, (q) => ({ ... }))`, **removed**), `q` here is the completed object — its key helpers are always safe to call. A per-call option still wins over whatever the default produces.

## Flatten

By default, accessors take the nested **envelope** params — `{ path?, query?, body? }`. Pass `flatten: true` to call them with **flat** params instead: the union of the request's `path`, `query`, and `body` fields.
...
This is purely a **call-site convenience** — `flatten` lives on `createQueries`, not `createApi`. The routar client and HTTP contract always use the envelope.

- **Off by default** — opt in per `createQueries`.
- **Endpoints that can't flatten fall back to the envelope automatically.** If two buckets collide on a key (e.g. both `path.id` and `body.id` exist) or the `body` is not a plain object (`z.array`, `z.string`, …), that accessor keeps the envelope shape — the types enforce this, so you can't accidentally write an ambiguous flat call.
- **The query key is always built from the normalized envelope.** `.queryKey()` / `.infinite.queryKey()` follow `flatten` too (same flat params as the accessor call), but internally they always normalize to the envelope before building the key — so SSR and CSR keys match no matter which call style each side uses. You can prefetch on the server with flat params and read on the client with flat params and the keys still converge. `.mutationKey` is unaffected (a static array, not a function).
