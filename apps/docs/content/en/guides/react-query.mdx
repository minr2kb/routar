...
- **The query key is always built from the envelope.** `.queryKey()` / `.mutationKey` / `.infinite.queryKey()` are flatten-independent, so SSR and CSR keys match no matter which call style each side uses. You can prefetch on the server with flat params and read on the client with flat params and the keys still converge.
