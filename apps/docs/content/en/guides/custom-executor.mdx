...
When your API wraps payloads in an envelope like `{ data: T }`, use the `unwrap` option instead of writing an `onResponse` plugin:
...
This is the clean alternative to the equivalent plugin:

```ts
// Works, but more boilerplate — prefer `unwrap`
...
`unwrap` runs as the innermost `onResponse` hook — immediately after the transport returns, before any plugins and before your `response` schema validates the payload. It is also available on `createFetchExecutor`, `@routar/axios`, and `@routar/ky`.
