...
Derives typed `queryOptions` and `mutationOptions` factories from a routar API client and router. The returned object mirrors the router's shape — GET endpoints become query accessors, non-GET endpoints become mutation accessors.
...
|-----------|------|-------------|
| `api` | `TApi` (routar API client) | The typed client produced by `createApi` |
| `router` | `RouterDef` | The router definition passed to `createApi` |
