...
## Why plugins, not middleware?

The classic middleware pattern (`(opts, next) => Promise`) works well for cross-lifecycle concerns like retry and timeout. But for the most common use cases — auth header injection, error handling, logging — related concerns get split across multiple functions:

```ts
// middleware approach: auth header injection and 401 refresh are separate
createExecutor(transport, [withAuth, withAuthRefresh])
```

Plugins bundle related concerns into a single named unit:

```ts
// plugin approach: auth header + 401 refresh in one place
...
`retry` and `timeout` remain as first-class options on `createFetchExecutor` because they require cross-lifecycle state that cannot be cleanly expressed as a plugin.
...
`retry` and `timeout` are available as options on `createFetchExecutor`, not as plugins. They require cross-lifecycle state (a timer that must be cleaned up in both success and error paths for timeout; a loop that re-calls `next` for retry) that cannot be expressed cleanly as `onRequest`/`onResponse`/`onError` hooks.
