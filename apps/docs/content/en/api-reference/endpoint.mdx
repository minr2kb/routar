---
title: endpoint()
description: Define a typed API endpoint with method, path, request schema, and response schema.
---

# `endpoint(spec)`

Defines a single endpoint. The primary purpose of this helper is to provide full type inference on `adapter` without manual casts — `raw` is automatically typed from the `response` schema.

```ts
import { endpoint } from '@routar/core'
import { z } from 'zod'

const TodoSchema = z.object({ id: z.number(), title: z.string() })

// without adapter
endpoint({
  method: 'GET',
  path: '/',
  response: z.array(TodoSchema),
})

// with adapter — raw is inferred as z.infer<typeof TodoSchema>[]
endpoint({
  method: 'GET',
  path: '/',
  response: z.array(TodoSchema),
  adapter: (raw) => raw.map(todo => ({ ...todo, label: todo.title })),
})
```

## Parameters

| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `method` | `'GET' \| 'POST' \| 'PUT' \| 'PATCH' \| 'DELETE'` | Yes | HTTP method |
| `path` | `string` | Yes | Path relative to the router prefix. Use `:param` for path parameters. |
| `response` | `AnyValidator<T>` | Yes | Schema for validating the response. Any object with `.parse(data): T`, or a [Standard Schema](#standard-schema). |
| `request` | `{ path?, query?, body? }` | No | The request schema — a [bucket-map](#request-buckets) of standalone validators, one per part. When `path` has `:param` segments, `request.path` is required. |
| `adapter` | `(raw: T) => U` | No | Transform the validated response. |

## Request buckets

`request` is a plain `{ path?, query?, body? }` map of standalone validators — one validator per part. routar composes the buckets into a single internal envelope, so call sites, query keys, react-query flatten, and MSW all see the same `{ path, query, body }` shape.

```ts
endpoint({
  method: 'GET',
  path: '/:id',
  request: {
    path: z.object({ id: z.number() }),
    query: z.object({ q: z.string() }),
  },
  response: TodoSchema,
})
```

Path-param enforcement still applies: when `path` has `:param` segments, `request.path` is required and must cover them.

## Standard Schema

`request`/`response` (and each bucket) accept any [Standard Schema](https://standardschema.dev) validator (the `~standard` interface — ArkType, Zod 3.24+, Valibot, …) in addition to any object with `.parse()`. At runtime routar prefers `.parse()` when present and otherwise calls `~standard.validate` (sync or async). Standard Schema issues are surfaced as a `StandardSchemaError` wrapped in `ValidationError`.

```ts
import { type } from 'arktype'

endpoint({
  method: 'GET',
  path: '/',
  response: type({ id: 'number', title: 'string' }).array(), // ArkType — no .parse
})
```

## Path param enforcement

If `path` contains `:param` segments, the `request` schema must include a matching `path` object — enforced at compile time.

```ts
// ✅ correct
endpoint({
  path: '/:id',
  request: { path: z.object({ id: z.number() }) },
  response: TodoSchema,
  method: 'GET',
})

// ❌ compile error — ':id' in path but request.path.id missing
endpoint({
  path: '/:id',
  request: { query: z.object({ q: z.string() }) },
  response: TodoSchema,
  method: 'GET',
})
```

## Adapter

`response` and `adapter` are always separate. `response` is a pure Zod schema (stays composable). `adapter` is a plain function that transforms the validated data.

```ts
// ✅ adapter separately — schema stays composable
const TodoRawSchema = z.object({ id: z.number(), title: z.string() })
const toTodoItem = (raw: z.infer<typeof TodoRawSchema>) => ({
  ...raw,
  label: raw.title,
})

endpoint({
  method: 'GET',
  path: '/',
  response: z.array(TodoRawSchema),
  adapter: (raw) => raw.map(toTodoItem), // raw is correctly typed
})
```

Never use `.transform()` on a `response` schema. `.transform()` wraps the schema in a `ZodEffects` object, which loses `.extend()`, `.merge()`, and all other composition methods — your schema can no longer be reused or composed. Put data transformations in `adapter` instead; the `response` schema stays a plain `ZodObject`.

## Type utilities

### `ApiTypes<TApi>`

Extracts request and response types from an API client. See the [dedicated `ApiTypes<T>` reference](/en/api-reference/api-types) for full usage and patterns.

```ts
import type { ApiTypes } from '@routar/core'

type TodoApiTypes = ApiTypes<typeof todoApi>
type CreateRequest  = TodoApiTypes['create']['request']  // { body: { title: string } }
type CreateResponse = TodoApiTypes['create']['response'] // Todo
```

### `PathParams<TPath>`

Extracts `:param` names from a path string as a union type.

```ts
import type { PathParams } from '@routar/core'

type P = PathParams<'/:userId/posts/:postId'> // 'userId' | 'postId'
```
