...
## In the AI era: less to keep consistent

Writing code is cheap now. AI generates fetch wrappers, types, and hooks at near-zero cost. But more generated code means more representations of the same API facts — and more places for drift when those facts change.

routar reduces the number of representations. One `endpoint()` derives the client function, types, query keys, and MSW mocks — automatically. Change the endpoint; edit one place, and everything downstream follows. See [AI Integration](/en/ai-integration) for how this plays out in practice.
