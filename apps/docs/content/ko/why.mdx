...
routar는 표현의 수를 줄입니다. 하나의 `endpoint()`에서 클라이언트 함수·타입·query key·MSW mock이 자동으로 파생됩니다. 엔드포인트를 변경하면 한 곳만 수정하면 되고, downstream이 전부 따라옵니다. [AI 통합](/ko/ai-integration)에서 실제로 어떻게 활용되는지 확인하세요.
