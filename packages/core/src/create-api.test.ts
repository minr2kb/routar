...
const failValidator = { parse: (_: unknown): unknown => { throw new Error('parse failed'); } };
...
describe('createApi', () => {
  describe('call signatures', () => {
    it('accepts RouterDef', async () => {
      ...
      const router = defineRouter('/todos', {
        getDetail: { method: 'GET' as const, path: '/:id', response: makeValidator({ id: 1 }) },
      });
    ...
    it('accepts prefix + endpoints (inline)', async () => {
      ...
      await api.list({});
      expect(executor.execute).toHaveBeenCalledWith(expect.objectContaining({ url: '/items' }));
    });
    ...
    it('accepts endpoints without prefix', async () => {
      ...
      await api.ping({});
      expect(executor.execute).toHaveBeenCalledWith(expect.objectContaining({ url: '/ping' }));
    });
  });
  ...
  describe('request handling', () => {
    ...
    it('passes signal to executor', async () => {
      ...
      expect(executor.execute).toHaveBeenCalledWith(expect.objectContaining({ signal }));
    });
    ...
    it('throws ValidationError when request.parse fails', async () => {
      const executor = mockExecutor({});
      const api = createApi(executor, '/items', {
        get: { method: 'GET' as const, path: '/', request: failValidator, response: makeValidator({}) },
      });
  ...
  describe('nested router', () => {
    ...
    it('prepends outer prefix via inline form', async () => {
      ...
      expect(executor.execute).toHaveBeenCalledWith(expect.objectContaining({ url: '/api/users' }));
    });
