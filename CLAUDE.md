...
- Per-call `headers`/`timeout`: query/mutation/`.infinite` accessors forward `headers`/`timeout` from their options object to the underlying endpoint call (core's `EndpointCallOptions` shape); also settable per-endpoint via `defaults`/`infinite` — call-site wins, `headers` shallow-merged over the default (call-site key wins on collision)
